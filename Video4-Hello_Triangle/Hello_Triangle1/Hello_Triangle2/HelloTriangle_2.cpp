...
int main()
{
	...
	glVertexAttribPointer(
		...
		(GLvoid*)0);						// Offset inside the structure to find the attribute

	// Color attribute, identified as "1"
	glVertexAttribPointer(
		1,									// Attribute index, "0".  The vertex shader should have layout "0" for the position of the vertex
		3,									// Number of components of attribute "0". In this case 3 floats for x,y,z
		GL_FLOAT,							// The data type of each component
		GL_FALSE,							// Normalize component values to [-1, 1]?
		sizeof(GLfloat) * 6,				// Stride, number of bytes between two instances of the attribute in the buffer
		(GLvoid*)(sizeof(GLfloat) * 3));	// Offset inside the structure to find the attribute
	...
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		glBindVertexArray(vao);
		glDrawArrays(GL_TRIANGLES, 0, 3); // draw points 0-2 from the currently bound VAO with current in-use shader
		glBindVertexArray(0);
